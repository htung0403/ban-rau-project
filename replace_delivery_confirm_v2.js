...
const regex = /static async confirmOrders\([^)]*\)\s*\{[\s\S]*?(?=\s*static async getInventory)/m;

const match = code.match(regex);
if (match) {
  const newCode = `static async confirmOrders(ids: string[]) {
  ...
  fs.writeFileSync(file, code.replace(match[0], newCode + '\n'));
  console.log('Successfully updated delivery.service.ts using regex');
} else {
