...
export const TimePicker24h: React.FC<Props> = ({ value, onChange, className }) => {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-[180px] p-2 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border-border bg-popover flex justify-between">
        <div className="flex-1 h-56 overflow-y-auto custom-scrollbar flex flex-col gap-1 pr-1 border-r border-border/50">
          <div className="sticky top-0 bg-popover/90 backdrop-blur-sm text-[11px] font-bold text-muted-foreground text-center py-1 pb-2">Giờ</div>
          {hours.map(h => {
            ...
            return (
              <button
                ...
                className={clsx(
                  ...
                  isSelected ? "bg-primary text-white font-bold" : "hover:bg-muted text-foreground"
                )}
              >
                {h}
              </button>
        ...
        <div className="flex-1 h-56 overflow-y-auto custom-scrollbar flex flex-col gap-1 pl-1">
          <div className="sticky top-0 bg-popover/90 backdrop-blur-sm text-[11px] font-bold text-muted-foreground text-center py-1 pb-2">Phút</div>
          {minutes.map(m => {
