...
const WarehousesPage: React.FC = () => {
  ...
  const filteredItems = inventoryItems.filter((item: any) => {
    ...
    const rName = orderData.receiver_name?.trim() || orderData.profiles?.full_name || orderData.customers?.name || '';
              ...
              <table className="w-full border-collapse min-w-[900px]">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-muted/30 border-b border-border">
                    ...
                    <th className="px-6 py-4 text-[11px] font-bold text-muted-foreground uppercase tracking-wider text-left">Chủ hàng</th>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1.5 text-[13px] font-bold text-slate-700">
                          ...
                          {(() => {
                            ...
                            return o.receiver_name?.trim() || o.profiles?.full_name || o.customers?.name || '-';
                          })()}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-[13px] font-medium text-slate-600">
                          {(() => {
                            const o = item.vegetable_orders || item.import_orders || {};
                            const sender = o.sender_name || o.customers?.name;
                            return sender || '-';
                          })()}
                        </div>
                      </td>
                    ...
                    {(() => {
                      ...
                      return sender ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] text-muted-foreground font-medium">Từ: {sender}</span>
                        </div>
                      ) : null;
                    })()}
                  </div>
