...
const SortableItem = ({ id, label, isVisible, onToggle }: { id: string, label: string, isVisible: boolean, onToggle: (id: string) => void }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center gap-2 p-2 bg-card rounded-lg border mb-1 shadow-sm transition-opacity ${isDragging ? 'opacity-50 border-primary' : 'border-transparent hover:border-border'}`}>
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          title="Tùy chỉnh cột"
          className="flex items-center justify-center border border-border bg-card text-muted-foreground hover:text-foreground hover:bg-muted/50 h-[38px] w-[38px] md:w-auto md:px-3 rounded-xl transition-colors shrink-0 shadow-sm"
        >
      ...
      <PopoverContent className="w-64 p-3 z-[100] border-transparent shadow-xl ring-1 ring-border/50" align="end">
        <div className="mb-3 border-b border-border pb-3">
          <h4 className="text-[14px] font-bold text-foreground">Tùy chỉnh cột</h4>
